...
class LibuvcConan(ConanFile):
    ...
    url = "https://github.com/kunaltyagi/conan-libuvc-flir"
    homepage = "https://github.com/groupgets/libuvc"
    ...
    def configure(self):
        if self.version != 'master':
            self.topics = self.topics + ("flir", "groupgets", "purethermal")
    ...
    def _patch(self):
        ...
        tools.replace_in_file(_cmakelists,
...
set(JPEG_LINK_FLAGS ${{{0}_LIBS}})'''
        ...
        if self.options.jpeg_turbo:
            tools.replace_in_file(_cmakelists, _jpg_find,
                                  _jpg_replace.format('libjpeg-turbo'))
        else:
            tools.replace_in_file(_cmakelists, _jpg_find,
                                  _jpg_replace.format('libjpeg'))
    ...
    def package(self):
        ...
        if self.options.shared:
            self.copy("libuvc.so", dst="lib", src=self._build_subfolder,
                      keep_path=False)
        else:
            name = "libuvc"
            suffix = "static" if self.version == "master" else ""
            lib = name + suffix + ".a"
            self.copy(lib, dst="lib", src=self._build_subfolder,
                      keep_path=False)
